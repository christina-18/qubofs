# ==============================================================================
# extract_pseudobulk_v9_mixed.R
# ------------------------------------------------------------------------------
# v9 mixed (2026-05-08): v8 の改良版. 14 cell types.
#
# v8 からの変更:
#   * Treg を CD4_other から分離 → 独立した "Treg" subtype に (biology 上の独立性)
#   * dnT と gdT を統合 → "dnT" subtype に (両者とも CD4-CD8- TCR group; 統合で
#     donor coverage が向上: gdT alone 13/18 → merged 18/18 in Heming HO)
#   * CD4_other (Naive + CTL + Proliferating, Treg 抜き) は試験的に維持:
#     - Heming HO で 8/32 donors とギリギリだが、まず含めて様子見
#     - QUBO で動かなければ後段で除外
#   * QC threshold 10→8 (CD4_other がギリギリ通るように緩和)
#
# Cell types (n = 14):
#   1.  B          (l2 aggregated)
#   2.  Mono       (CD14 + CD16)
#   3.  NK         (NK_1-4 + NK_CD56bright)
#   4.  dnT        (NEW MERGED: dnT_1+2 + gdT_1-4)
#   5.  Treg       (NEW SPLIT: Treg Memory + Treg Naive)
#   6.  CD4_TCM    (TCM_1+2+3)
#   7.  CD4_TEM    (TEM_1+2+3+4)
#   8.  CD4_other  (Naive + CTL + Proliferating, Treg 抜き)
#   9.  CD8_TEM    (TEM_1-6)
#   10. CD8_TCM    (TCM_1+2+3)
#   11. CD8_other  (Naive + Naive_2 + Proliferating + MAIT)
#   12. cDC1
#   13. cDC2       (cDC2_1 + cDC2_2)
#   14. pDC
#
# 想定ランタイム: ~30-60 min/cohort
# ==============================================================================
...
OUT_BASE <- "/Users/mizuhoasada/Documents/Claude/Projects/MS_scRNA_GeneSelection_QUBO/data/pseudobulk_v9_mixed"
...
# --- v9 mixed 細胞型グループ (selective aggregation, 14 types) ------------------
# Changes from v8:
#   - dnT and gdT merged (both CD4-CD8- TCR groups, more donors when combined)
#   - Treg separated from CD4_other (distinct immunoregulatory program)
#   - CD4_other (without Treg) kept as borderline subtype — drop downstream if
...
CELL_TYPES <- list(
  ...
  # NEW MERGED: dnT_1+2 + gdT_1-4 (was 2 separate types in v8)
  dnT        = c("dnT_1", "dnT_2",
                 "gdT_1", "gdT_2", "gdT_3", "gdT_4"),
  # NEW SPLIT: Treg as its own type (was inside CD4_other in v8)
  Treg       = c("Treg Memory", "Treg Naive"),
  # CD4_T splits
  CD4_TCM    = c("CD4 TCM_1", "CD4 TCM_2", "CD4 TCM_3"),
  CD4_TEM    = c("CD4 TEM_1", "CD4 TEM_2", "CD4 TEM_3", "CD4 TEM_4"),
  CD4_other  = c("CD4 Naive", "CD4 CTL", "CD4 Proliferating"),  # Treg 抜き
  # CD8_T splits (unchanged from v8)
...
for (ct_name in names(CELL_TYPES)) {
  ...
  for (tissue in TISSUES) {
    ...
    if (length(qc_donors) < 8) {  # v9: 10 -> 8 (CD4_other がギリギリ通るため緩和)
