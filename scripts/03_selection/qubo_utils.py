...
qubo_utils.py
=============
Utility functions for the per-cell-type QUBO pipeline: fold loading, QUBO
matrix construction, simulated-annealing solving, cohort-consistency scoring,
and lightweight classifiers / metrics.

Pure numpy / pandas — no scikit-learn dependency for the classifier core.
"""
...
def load_pb(fold_dir: Path, prefix: str) -> Tuple[np.ndarray, List[str], List[str]]:
    ...
    Deduplicates donors by first occurrence as a safety check.
    """
    ...
    # dedupe columns (safety: ensure each donor appears once)
    seen = {}
    keep_idx = []
    unique_cols = []
...
def load_fold(data_root: Path, cell_type: str, tissue: str, fold: int,
    ...
    deg_source: "lm" (→ tstats.csv) | "deseq2" | "edger" | "limmavoom"
        | "deseq2_counts" | "edger_counts" | "limmavoom_counts"
        The "_counts" variants read DE statistics computed from donor-level
        SoupX-corrected count-sum pseudobulk (tstats_<m>_counts.csv) and are the
        manuscript-consistent sources (edgeR primary; see docs/reproduction.md).
        Any value other than "lm" maps to tstats_<deg_source>.csv.
...
def cohort_variance_per_gene(
    ...
    NOTE: this function is retained for development / legacy analyses and is NOT
    used in the canonical manuscript relevance score, which is the
    cohort-consistency-weighted edgeR statistic |z_i|*C_i (see
...
def cohort_consistency_per_gene(
    bundles: Dict[str, Dict],
    candidate_genes: List[str],
) -> np.ndarray:
    """Directional cohort-consistency score C_i (manuscript |z_i|*C_i weighting).

    For each candidate gene, C_i is the fraction of informative training cohorts
    whose MS-versus-control log-fold-change agrees with the majority direction:

        C_i = max(n_pos, n_neg) / n_total   over cohorts with a non-zero sign,

    where the per-cohort log-FC is mean(MS donors) - mean(control donors) on the
    training-donor pseudobulk. Cohorts lacking both classes are skipped. If fewer
    than two informative cohorts are available, C_i = 1.0 (consistency cannot be
    assessed). Scores are averaged across cell types when multiple are given.
    Port of scripts/qubo_step1_cohort_consistency.py (per_cohort_logFC +
    cohort_consistency_score). Returns (n_genes,) in [0, 1]; default 1.0.
    """
    n = len(candidate_genes)
    per_ct = []
    for ct, b in bundles.items():
        ...
        tr = b["train"]
        meta = tr.get("meta")
        donors = tr.get("donors")
        if meta is None or donors is None or "cohort" not in meta.columns \
                or "diagnosis" not in meta.columns:
            continue
        d2c = dict(zip(meta["donor_id"], meta["cohort"]))
        d2dx = dict(zip(meta["donor_id"], meta["diagnosis"]))
        cohorts = np.array([d2c.get(d, "unknown") for d in donors])
        dx = np.array([d2dx.get(d, "NA") for d in donors])
        gene_pos = {g: i for i, g in enumerate(tr["genes"])}
        X = tr["X"]
        # per-cohort log-FC for informative cohorts (>=1 MS and >=1 control)
        logfc_cols = []
        for c in np.unique(cohorts):
            ms = (cohorts == c) & (dx == "MS")
            hd = (cohorts == c) & (dx == "HD")
            if ms.sum() < 1 or hd.sum() < 1:
                continue
            col = np.full(n, np.nan)
            for j, g in enumerate(candidate_genes):
                ip = gene_pos.get(g, -1)
                if ip >= 0:
                    col[j] = float(X[ip, ms].mean()) - float(X[ip, hd].mean())
            logfc_cols.append(col)
        if len(logfc_cols) < 2:
            # cannot enforce consistency for this cell type
            per_ct.append(np.ones(n))
            continue
        L = np.vstack(logfc_cols)          # cohorts x genes
        signs = np.sign(L)
        n_pos = np.nansum(signs > 0, axis=0)
        n_neg = np.nansum(signs < 0, axis=0)
        n_tot = np.nansum(signs != 0, axis=0)
        C = np.where(n_tot > 0, np.maximum(n_pos, n_neg) / np.maximum(n_tot, 1), 0.0)
        per_ct.append(C.astype(np.float64))
    if not per_ct:
        return np.ones(n)
    return np.nanmean(np.vstack(per_ct), axis=0)
...
def build_score_and_redundancy(
    ...
    for ct, b in bundles.items():
        ...
        if present_mask.sum() >= 2 and sub.shape[1] >= 3:
            sub_present = sub[present_mask, :].astype(np.float64)
            # (near-)zero variance rows -> treated as constant -> 0 corr.
            # NOTE: use a small relative floor, not `== 0`. A gene that is
            # near-constant across training donors (std ~1e-12 but not exactly 0)
            # would otherwise be divided by a tiny std, producing huge z-scores
            # that overflow in the matmul below (the source of the historical
            # "invalid value encountered in matmul" RuntimeWarning). Flooring the
            # std collapses such a row to ~0, i.e. no linear correlation, which is
            # the correct neutral treatment for a constant gene.
            ...
            # guard against any residual non-finite value (degenerate genes) ->
            # 0 redundancy, so the QUBO matrix is never contaminated by NaN/inf.
            c = np.nan_to_num(np.clip(c, -1.0, 1.0), nan=0.0, posinf=0.0, neginf=0.0)
...
def roc_auc(y_true, y_score):
    """Mann-Whitney ROC-AUC with average-rank handling for tied scores
    (matches scikit-learn's roc_auc_score). Returns NaN if either class is empty."""
    ...
    if n_pos == 0 or n_neg == 0:
        return np.nan
    # average ranks (1-based); tied scores share the mean of their rank block
